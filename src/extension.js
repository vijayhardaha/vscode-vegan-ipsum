...
function activate(context) {
	var commands = [
		...
		vscode.commands.registerCommand(
			"vegan-ipsum.paragraph",
			generateParagraph
		),
		vscode.commands.registerCommand(
			"vegan-ipsum.multipleParagraphs",
...
function generateLine() {
	insertText({ count: 1, units: "sentences" });
}
...
function generateParagraph() {
	insertText({ count: 1, units: "paragraphs" });
}
...
async function generateMultipleParagraphs() {
	...
	insertText({ count: Number.parseInt(count), units: "paragraphs" });
}
